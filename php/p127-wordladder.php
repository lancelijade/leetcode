...
class Solution {
    ...
    function ladderLength($beginWord, $endWord, $wordList) {
        ...
        $graph = $this->buildGraph($beginWord, $wordList);
        //$this->output($graph, 0, count($graph)-1);
        $n = $this->Dijkstra($graph, 0, count($graph))+1;
    ...
    function buildGraph($begin, $list) {
        ...
        // init graph
        $graph = array_fill(0, $d, array());
        for ($i=0; $i<$d; $i++) {
            //$graph[$i] = array_fill(0, $d, 0);
            $graph[$i] = str_repeat(0, $d);
        }
        //$this->output($graph, 0, $d-1);
